...
from app.models import AudioCheckRequest, CheckResponse, ImageCheckRequest, TextCheckRequest
from app.moderation import moderate_text

app = FastAPI(title="safecomms API", version="2.0.0")
...
def check_image(payload: ImageCheckRequest) -> CheckResponse:
    safe, category, matched_terms, reason = moderate_text(payload.description)
    return CheckResponse(safe=safe, category=category, matched_terms=matched_terms, reason=reason)
...
def check_text_ai(payload: TextCheckRequest, threshold: float = Query(default=0.5, ge=0.0, le=1.0)) -> CheckResponse:
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Local AI model unavailable: {exc}",
        ) from exc
